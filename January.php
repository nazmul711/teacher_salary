	...
	<section class="salary-section">
       ...
       <div class="postion_salay">
           ...
           <div class="maincontain">
               ...
               <h2 class="salary-table">January-Salary Sheet</h2>
			   <?php 
			   ...
			   ?>
				...
				<table class="ts_tbl">
					   ...
					   <?php 
						$query = "SELECT * FROM january ORDER BY id";
						$query_data = $db->select($query);
						if($query_data==true){
							$x = 0;
							$i;
                       ...
                       <tr>
						  ...
						  <td><input type="number" name="T_S_I" value="<?php
																		$UP = "UPDATE january SET Total_increse = (incre_Yearly + Last_Y_increse)"; 
																		$UP_R = $db->update($UP); 
						   ...
						   <td><input type="number" name="T_S_I" value="<?php 
																			$UP = "UPDATE january SET total_Honararia = (2nd_shift_fee + incre_Honoraria)"; 
																			$UP_R = $db->update($UP); 
						   ...
						   <td><input type="number" name="T_S_I" value="<?php 
																		$UP = "UPDATE january SET Total_Salary = (Orgin_Salary + Total_increse + H_rent + Treatment + Vehicular +total_Honararia + Festival_bonus)"; 
																		$UP_R = $db->update($UP); 
																		echo $query_r['Total_Salary'];?>" ></td>
                           <td><input type="number" name="T_S_I" value="<?php echo $query_r['F_Found'];?>" ></td>
						   <td><input type="number" name="T_S_I" value="<?php 
																			$UP = "UPDATE january SET Given_salary = (Total_Salary - F_Found)"; 
																			$UP_R = $db->update($UP);
																			echo $query_r['Given_salary'];?>" ></td>
						   <td><a href="#">Report</a><br><a href="T_Edit_Jan.php?edit_T=<?php echo $query_r['id'];?>">Edit</a></td>
                       </tr>
					   <?php } }?>
					   
					   <tr class="tolal_row">
					   <?php
						$query_t = "select Given_salary from january";
